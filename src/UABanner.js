...
(function() {
  ...
  // little helper-function to get params from an object
  ...
  window.UABanner = function UABanner(title, text, timeout, buttons, width) {
    ...
    var domElement = document.createElement("div").setClass("uakit-banner");
    domElement.style.width = this.width + "px";
    var contentElement = document.createElement("div").setClass("uakit-content").setContent([
      // Titel
      (function() {
        var e = document.createElement("p").setClass("uakit-title");
        if (!t.text)
      ...
      !!t.text ? document.createElement("p").setClass("uakit-text").setContent(
        this.text.toTextNode()
      ) : undefined
    ]);

    // Controls-Elements (Buttons)
    var controlsElement = document.createElement("div").setClass("uakit-controls").setContent([
      (function() {
        if (!t.buttons)
          return;
    ...
    if (this.buttons.length > 0)
      elements.push([controlsElement, "uakit-controls-cell"]); // [element, class], just for debugging

    var contentTable = table(elements).setClass("uakit-content-table");
